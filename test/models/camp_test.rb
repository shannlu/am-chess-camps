...
class CampTest < ActiveSupport::TestCase
  ...
  context "Within context" do
    ...
    should "verify that the camp's curriculum is active in the system" do
      ...
      bad_camp = FactoryBot.build(:camp, curriculum: @smithmorra, location: @cmu, start_date: Date.new(2019,8,1), end_date: Date.new(2019,8,5))
      deny bad_camp.valid?
      # test the nonexistent curriculum
      gambit = FactoryBot.build(:curriculum, name: "King's Gambit")
      gambit_camp = FactoryBot.build(:camp, curriculum: gambit, location: @cmu, start_date: Date.new(2019,8,1), end_date: Date.new(2019,8,5))
    ...
    should "verify that the camp's location is active in the system" do
      ...
      bad_camp = FactoryBot.build(:camp, curriculum: @tactics, location: @sqhill, start_date: Date.new(2019,8,1), end_date: Date.new(2019,8,5))
      deny bad_camp.valid?
      delete_inactive_locations
      # test the nonexistent location
      bhill = FactoryBot.build(:location, name: "Blueberry Hill")
      bhill_camp = FactoryBot.build(:camp, curriculum: @tactics, location: bhill, start_date: Date.new(2019,8,1), end_date: Date.new(2019,8,5))
    ...
    should "have 'full' and 'empty' scopes to find camps at max and at 0, respectively" do
      create_family_users
      create_families
      create_students
      create_registrations
      # show two camps are empty and none are full
    ...
    should "have 'is_full?' and 'enrollment' instance methods" do
      create_family_users
      create_families
      create_students
      create_registrations
      ...
      assert_equal 3, @camp4.enrollment
      # reduce max students of camp 4 so it will register as full
      @camp4.max_students = 3
      @camp4.save
    ...
    should "shows that a camp with same date and time slot but different location can be created" do
      @ok_camp = FactoryBot.build(:camp, curriculum: @tactics, location: @north, start_date: Date.new(2022,7,23), end_date: Date.new(2022,7,27), time_slot: 'am')
      assert @ok_camp.valid?
    end

    should "shows that a duplicate camp (same date, time and location) cannot be created" do
      @bad_camp = FactoryBot.build(:camp, curriculum: @tactics, location: @cmu, start_date: Date.new(2022,7,23), end_date: Date.new(2022,7,27), time_slot: 'am')
      deny @bad_camp.valid?
    end
    ...
    should "remove not instructors from inactive camps" do
      create_users
      create_instructors
    ...
    should "have a method for finding active camps that still have openings" do
      ...
      small_camp = FactoryBot.create(:camp, curriculum: @endgames, start_date: 8.weeks.from_now.to_date, end_date: 9.weeks.from_now.to_date, time_slot: "pm", location: @cmu, max_students: 2)
      zach_endgames  = FactoryBot.create(:registration, camp: small_camp, student: @zach)
      kelsey_endgames = FactoryBot.create(:registration, camp: small_camp, student: @kelsey)
